...
const Supplier = require("../../models/InventoryModels/Supplier");
...
const requestBody = Joi.object({
  ...
  status: Joi.string()
    .valid("draft", "sent", "confirmed", "received", "cancelled", "closed")
    .optional(),
  ...
  if (value.sendEmail && supplier.email) {
    try {
      const itemsHtml = value.items
        .map(
          (i) =>
            `<li>${i.name} – ${i.quantity}${i.unit ? " " + i.unit : ""}</li>`
        )
        .join("");
      await sendEmail({
        to: supplier.email,
...
router.patch("/:id/status", async (req, res) => {
  ...
  if (body.error)
    throw Object.assign(new Error(body.error.message), { status: 400 });
